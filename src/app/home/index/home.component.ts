...
export class HomeComponent implements OnInit {
  ...
  constructor(private pageScrollService: PageScrollService, private homeService: HomeService) {
    this.homeService.get('HomePages').subscribe((data: IHome[]) => {
      this.pages = data;
    });

    this.homeService.get('UkInsuranceAccounts').subscribe((data: IAccount[]) => {
    ...
    if (name && this.seletedAccount) {
      switch (name) {
        case UkInsuranceAccounts.FNZ: {
          return this.getContributionTexts(this.seletedAccount.FNZ);
        }
        case UkInsuranceAccounts.RLG: {
          return this.getContributionTexts(this.seletedAccount.RLG);
        }
        case UkInsuranceAccounts.SLA: {
          return this.getContributionTexts(this.seletedAccount.SLA);
        }
        case UkInsuranceAccounts.Aegon: {
          return this.getContributionTexts(this.seletedAccount.Aegon);
        }
      }
    }
  }

  getContributionTexts(text: string): string[] {
    if (text && text.length > 0) {
      let contributions = text.split(';');
      return contributions.filter(text => text && text.length > 0);
    }
    return [];
  }
}
